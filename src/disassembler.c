...
unsigned char DisassembleOpcode(unsigned short buffer, unsigned short pc)
{
    ...
    else if((code & 0xF000) == 0x0000) printf("SYS        $%03X", code & 0x0FFF);
    else if((code & 0xF000) == 0x1000) printf("JP         $%03X", code & 0x0FFF);
    else if((code & 0xF000) == 0x2000) printf("CALL       $%03X", code & 0x0FFF);
    else if((code & 0xF000) == 0x3000) printf("SE    V%X,  #$%02X", (code & 0x0F00) >> 8, code & 0x00FF);
    else if((code & 0xF000) == 0x4000) printf("SNE   V%X,  #$%02X", (code & 0x0F00) >> 8, code & 0x00FF);
    else if((code & 0xF00F) == 0x5000) printf("SE    V%X,  V%X", (code & 0x0F00) >> 8, (code & 0x00F0) >> 4);
    else if((code & 0xF000) == 0x6000) printf("LD    V%X,  #$%02X", (code & 0x0F00) >> 8, code & 0x00FF);
    else if((code & 0xF000) == 0x7000) printf("ADD   V%X,  #$%02X", (code & 0x0F00) >> 8, code & 0x00FF);
    ...
    else if((code & 0xF000) == 0xA000) printf("LD    I,    $%03X", code & 0x0FFF);
    else if((code & 0xF000) == 0xB000) printf("JP    V0,   $%03X", code & 0x0FFF);
    else if((code & 0xF000) == 0xC000) printf("RND   V%X,  #$%02X", (code & 0x0F00) >> 8, code & 0x00FF);
    else if((code & 0xF000) == 0xD000) printf("DRW   V%X,  V%X,  #$%01X", (code & 0x0F00) >> 8, (code & 0x00F0) >> 4, code & 0x000F);
